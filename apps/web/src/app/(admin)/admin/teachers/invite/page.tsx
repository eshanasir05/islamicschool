...
const STATUS_MESSAGES: Record<string, { text: string; variant: 'success' | 'warn' | 'error' }> = {
  invited: {
    text: 'Invite sent! The teacher will receive an email with a sign-in link.',
    variant: 'success',
  },
  added: {
    text: 'Teacher already has an account — added to this school as a teacher.',
    variant: 'success',
  },
...
export default async function InviteTeacherPage({ searchParams }: Props) {
  ...
  return (
    <main className="app-main">
      <Breadcrumb items={[{ label: 'Teachers', href: '/admin/teachers' }, { label: 'Invite' }]} />
      ...
      {statusInfo && <div className={`banner banner-${statusInfo.variant}`}>{statusInfo.text}</div>}
      ...
      >
        <strong style={{ color: 'var(--fg)' }}>How it works:</strong> The teacher receives a sign-in
        link from Supabase. Clicking it logs them in directly and lands them on the teacher
        dashboard. They can set a password later using &ldquo;Forgot password&rdquo; on the sign-in
        page.
      </div>
    </main>
  );
